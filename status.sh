...
ayao_prefix="https://jsd.cdn.zzko.cn/gh/CokeMine/ServerStatus-Hotaru@master"
coding_prefix="https://cokemine.coding.net/p/hotarunet/d/ServerStatus-Hotaru/git/raw/master"
link_prefix=${ayao_prefix}
...
Download_Server_Status_server() {
  cd "/tmp" || exit 1
  [[ ${mirror_num} == 2 ]] && bundle_link="https://cokemine.coding.net/p/hotarunet/d/ServerStatus-Hotaru/git/archive/master/?download=true" || bundle_link="https://jsd.cdn.zzko.cn/gh/CokeMine/ServerStatus-Hotaru/archive/master.zip"
  [[ ${mirror_num} == 2 ]] && ayao_link="https://jsd.cdn.zzko.cn/gh" || ayao_link="https://jsd.cdn.zzko.cn/gh"
  ...
  if [[ -e "${server_file}/sergate" ]]; then
    ...
    mv "/tmp/ServerStatus-Hotaru-master/server/sergate" "${server_file}/sergate"
    wget -N --no-check-certificate "${ayao_link}/cokemine/hotaru_theme/releases/latest/download/hotaru-theme.zip"
...
Install_jq() {
  [[ ${mirror_num} == 2 ]] && {
    ayao_link="https://hub.fastgit.org"
    raw_link="https://raw.fastgit.org"
  } || {
    ayao_link="https://jsd.cdn.zzko.cn/gh"
    raw_link="https://jsd.cdn.zzko.cn/gh"
  }
  if [[ ! -e ${jq_file} ]]; then
    if [[ ${bit} == "x86_64" ]]; then
      jq_file="${file}/jq"
      wget --no-check-certificate "${ayao_link}/stedolan/jq/releases/download/jq-1.5/jq-linux64" -O ${jq_file}
    elif [[ ${bit} == "i386" ]]; then
      jq_file="${file}/jq"
      wget --no-check-certificate "${ayao_link}/stedolan/jq/releases/download/jq-1.5/jq-linux32" -O ${jq_file}
    else
...
Update_Shell() {
  ...
  [[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 ayao !" && exit 0
...
Set_Mirror() {
  echo -e "${Info} 请输入要选择的下载源，默认使用ayao，2号故障
  ${Green_font_prefix} 1.${Font_color_suffix} ayao
  ...
  [[ ${mirror_num} == 2 ]] && link_prefix=${coding_prefix} || link_prefix=${ayao_prefix}
}
check_sys
action=$1
if [[ -n $action ]]; then
